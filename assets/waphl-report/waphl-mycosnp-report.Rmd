...
# tree files
fasttree_file <- "fasttree_phylogeny.tre"
rapidnj_file <- "rapidnj_phylogeny.tre"
...
display_qc <- function(qc_file){
  if(file.exists(qc_file)){
    ...
    # determine QC status
    df <- df %>%
      mutate(gc_per = as.numeric(str_remove_all(GC, pattern = "%"))) %>%
      mutate("Quality Status" = case_when(gc_per < 42 | gc_per > 47.5 | `Average Q Score` < 28 | `Mean Coverage Depth` < 20 ~ "FAIL",
                                          TRUE ~ "PASS")) %>%
      select(-gc_per)
...
if(file.exists(snpeff_file)){
  dev_null <- snpeff_report(snpeff_file)
}
```
\newpage  
